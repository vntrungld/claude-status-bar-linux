...
**Interfaces:**
- Consumes: `clawdAnim` from `shared/labels.mjs`; `shared/clawd/*.png` and `frames.mjs`
...
class ClawdSprite extends St.Bin {
    ...
    setState(state, tool) {
        ...
        // Frame dimensions come from frames.mjs; Animation derives the frame
        // count from the sheet width, which matches how the sheets are built.
        // KDE disables frame interpolation so playback steps discretely — this
        // path already blits one clip rect per frame, which is the same result.
...
sheets and the same frames.mjs interval the plasmoid uses, so both
