...
class UsageBar extends St.BoxLayout {
    _init(label) {
        ...
        // the standard shell idiom. x_expand/y_expand must be pinned false on
        // the trough -- an St.BoxLayout child otherwise stretches into
        // whatever spare width AND height the row has (here, the popup's
        // min-width: 300px, and the row's height set by the taller name
        // label), overriding the trough's CSS width/height and rendering it
        // far too wide, or in this case taller than the fill sitting inside
        // it. Same failure mode ClawdSprite hit; see that fix's commit for
        // the full explanation.
        this._trough = new St.Widget({
            style_class: 'claude-usage-trough',
